...
use knob::KnobValue;
use update::Update;
use clock_network::{ClockNetwork, ClockNodePrototype};
use super::*;
...
fn test_basic_clock() {
    ...
    let proto = Clock::get_prototype();
    let node_id = network.add_node(proto, "test", []).unwrap().id;

    asseert!(INIT_RATE.in_hz() == 1.0);
...
fn test_multiplier() {
    let mut network = ClockNetwork::new();
    let bp, mp = Clock::get_prototype(), ClockMultiplier::get_prototype();
    let basic_id = network.add_node(bp, "basic", []).unwrap().id;
    let mult_id = network.add_node(mp, "mult", vec!(basic_id)).unwrap().id;
